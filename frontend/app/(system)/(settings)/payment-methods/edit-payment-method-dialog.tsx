...
import { updatePaymentMethodSchema, type UpdatePaymentMethodFormData } from '@/lib/validations';
import { PaymentMethod, ApiError } from '@/types/api';
...
export function EditPaymentMethodDialog({ item, open, onOpenChange }: Props) {
  const mutation = useUpdatePaymentMethod();

  const form = useForm<UpdatePaymentMethodFormData>({
  ...
  const onSubmit = (data: UpdatePaymentMethodFormData) => {
    if (!item) return;
    mutation.mutate(
      { id: item.id, data },
      {
        ...
        onError: (error) => {
          ...
          if (axiosError.response?.data?.errors) {
            Object.entries(axiosError.response.data.errors).forEach(([key, value]) => {
              form.setError(key as keyof UpdatePaymentMethodFormData, {
                message: Array.isArray(value) ? value[0] : value,
              });
            });
          } else {
            form.setError('root', {
              message: axiosError.response?.data?.message || 'Failed to update payment method',
            });
          }
        },
      }

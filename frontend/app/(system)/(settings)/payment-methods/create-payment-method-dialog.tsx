...
import { createPaymentMethodSchema, type CreatePaymentMethodFormData } from '@/lib/validations';
...
export function CreatePaymentMethodDialog({ open, onOpenChange }: Props) {
  ...
  const onSubmit = (data: CreatePaymentMethodFormData) => {
      ...
      onError: (error) => {
        ...
        if (axiosError.response?.data?.errors) {
          Object.entries(axiosError.response.data.errors).forEach(([key, value]) => {
            form.setError(key as keyof CreatePaymentMethodFormData, {
              message: Array.isArray(value) ? value[0] : value,
            });
          });
        } else {
          form.setError('root', {
            message: axiosError.response?.data?.message || 'Failed to create payment method',
          });
        }
      },
    });
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) form.reset(); onOpenChange(v); }}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Add a new payment method option for merchants.</DialogDescription>
        </DialogHeader>
        <Form {...form}>

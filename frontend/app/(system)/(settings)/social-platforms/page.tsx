...
import { useSocialPlatforms, useDeleteSocialPlatform } from '@/hooks/useSocialPlatforms';
import { SocialPlatform, SocialPlatformQueryParams } from '@/types/api';
...
import {
  ChevronLeft, ChevronRight, MoreHorizontal, Plus, Pencil, Trash2, Share2, RefreshCw,
} from 'lucide-react';
import { CreateSocialPlatformDialog } from './create-social-platform-dialog';
import { EditSocialPlatformDialog } from './edit-social-platform-dialog';
...
export default function SocialPlatformsPage() {
  ...
  const [editItem, setEditItem] = useState<SocialPlatform | null>(null);
  const [deleteItem, setDeleteItem] = useState<SocialPlatform | null>(null);

  const queryParams = useMemo<SocialPlatformQueryParams>(() => {
    const params: SocialPlatformQueryParams = { page, per_page: perPage };
  ...
  const { data, isLoading, refetch, isFetching } = useSocialPlatforms(queryParams);
  const deleteMutation = useDeleteSocialPlatform();
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage social media platforms for merchant profiles</p>
        </div>
        <PermissionGate permission="social_platforms.create">
          <Button onClick={() => setCreateDialogOpen(true)} size="lg">
            <Plus className="mr-2 h-4 w-4" /> Add Social Platform
          </Button>
        </PermissionGate>
      </div>
        ...
        <CardHeader className="border-b">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <CardTitle>All Social Platforms</CardTitle>
                <CardDescription>Manage predefined social media platforms.</CardDescription>
              </div>
              <Button variant="outline" size="icon" onClick={() => refetch()} disabled={isFetching}>
                <RefreshCw className={`h-4 w-4 ${isFetching ? 'animate-spin' : ''}`} />
              </Button>
            </div>
            <DataTableFilters filters={filters} values={filterValues} onChange={handleFilterChange} onReset={handleFilterReset} globalSearchKey="search" globalSearchPlaceholder="Search social platforms..." />
          </div>
        </CardHeader>
              ...
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} className="h-32">
                    <div className="flex flex-col items-center justify-center text-center">
                      <Share2 className="h-10 w-10 text-muted-foreground/50 mb-2" />
                      <p className="text-muted-foreground font-medium">No social platforms found</p>
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
                data?.data?.map((item) => (
                  <TableRow key={item.id} className="group">
                    ...
                    <TableCell className="text-muted-foreground max-w-[250px] truncate">{item.base_url}</TableCell>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <PermissionGate permission="social_platforms.update"><DropdownMenuItem onClick={() => setEditItem(item)}><Pencil className="mr-2 h-4 w-4" /> Edit</DropdownMenuItem></PermissionGate>
                          <PermissionGate permission="social_platforms.delete"><DropdownMenuSeparator /><DropdownMenuItem onClick={() => setDeleteItem(item)} className="text-destructive focus:text-destructive"><Trash2 className="mr-2 h-4 w-4" /> Delete</DropdownMenuItem></PermissionGate>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
                  </TableRow>
      ...
      <CreateSocialPlatformDialog open={createDialogOpen} onOpenChange={setCreateDialogOpen} />
      <EditSocialPlatformDialog item={editItem} open={!!editItem} onOpenChange={(open) => !open && setEditItem(null)} />

      <AlertDialog open={!!deleteItem} onOpenChange={(open) => !open && setDeleteItem(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Delete Social Platform</AlertDialogTitle><AlertDialogDescription>Are you sure you want to delete <span className="font-semibold">{deleteItem?.name}</span>?</AlertDialogDescription></AlertDialogHeader>

...
import { useCreateBusinessType } from '@/hooks/useBusinessTypes';
import { createBusinessTypeSchema, type CreateBusinessTypeFormData } from '@/lib/validations';
...
export function CreateBusinessTypeDialog({ open, onOpenChange }: Props) {
  ...
  const onSubmit = (data: CreateBusinessTypeFormData) => {
      ...
      onError: (error) => {
        ...
        if (axiosError.response?.data?.errors) {
          Object.entries(axiosError.response.data.errors).forEach(([key, value]) => {
            form.setError(key as keyof CreateBusinessTypeFormData, { message: Array.isArray(value) ? value[0] : value });
          });
        } else {
          form.setError('root', { message: axiosError.response?.data?.message || 'Failed to create business type' });
        }
      },
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) form.reset(); onOpenChange(v); }}>
      <DialogContent>
        <DialogHeader><DialogTitle>Create Business Type</DialogTitle><DialogDescription>Add a new business type category.</DialogDescription></DialogHeader>
        <Form {...form}>

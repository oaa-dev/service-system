...
import { useBusinessTypes, useDeleteBusinessType } from '@/hooks/useBusinessTypes';
import { BusinessType, BusinessTypeQueryParams } from '@/types/api';
...
import {
  ChevronLeft, ChevronRight, MoreHorizontal, Plus, Pencil, Trash2, Briefcase, RefreshCw,
} from 'lucide-react';
import { CreateBusinessTypeDialog } from './create-business-type-dialog';
import { EditBusinessTypeDialog } from './edit-business-type-dialog';
...
export default function BusinessTypesPage() {
  ...
  const [deleteItem, setDeleteItem] = useState<BusinessType | null>(null);

  const queryParams = useMemo<BusinessTypeQueryParams>(() => {
    const params: BusinessTypeQueryParams = { page, per_page: perPage };
  ...
  const { data, isLoading, refetch, isFetching } = useBusinessTypes(queryParams);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage business type categories for merchants</p>
        </div>
        <PermissionGate permission="business_types.create">
          <Button onClick={() => setCreateDialogOpen(true)} size="lg">
            <Plus className="mr-2 h-4 w-4" /> Add Business Type
          </Button>
        </PermissionGate>
      </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <CardDescription>Manage predefined business categories for merchants.</CardDescription>
              </div>
              <Button variant="outline" size="icon" onClick={() => refetch()} disabled={isFetching}>
                <RefreshCw className={`h-4 w-4 ${isFetching ? 'animate-spin' : ''}`} />
              </Button>
            </div>
            <DataTableFilters filters={filters} values={filterValues} onChange={handleFilterChange} onReset={handleFilterReset} globalSearchKey="search" globalSearchPlaceholder="Search business types..." />
          </div>
        </CardHeader>
                ...
                <TableRow>
                  <TableCell colSpan={6} className="h-32">
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-muted-foreground font-medium">No business types found</p>
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <PermissionGate permission="business_types.update"><DropdownMenuItem onClick={() => setEditItem(item)}><Pencil className="mr-2 h-4 w-4" /> Edit</DropdownMenuItem></PermissionGate>
                          <PermissionGate permission="business_types.delete"><DropdownMenuSeparator /><DropdownMenuItem onClick={() => setDeleteItem(item)} className="text-destructive focus:text-destructive"><Trash2 className="mr-2 h-4 w-4" /> Delete</DropdownMenuItem></PermissionGate>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
                  </TableRow>
      ...
      <CreateBusinessTypeDialog open={createDialogOpen} onOpenChange={setCreateDialogOpen} />
      <EditBusinessTypeDialog item={editItem} open={!!editItem} onOpenChange={(open) => !open && setEditItem(null)} />

      <AlertDialog open={!!deleteItem} onOpenChange={(open) => !open && setDeleteItem(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Delete Business Type</AlertDialogTitle><AlertDialogDescription>Are you sure you want to delete <span className="font-semibold">{deleteItem?.name}</span>?</AlertDialogDescription></AlertDialogHeader>

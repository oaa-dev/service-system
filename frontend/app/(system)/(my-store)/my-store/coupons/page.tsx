...
import { useMyCoupons, useCreateMyCoupon, useUpdateMyCoupon, useDeleteMyCoupon } from '@/hooks/useCoupons';
...
export default function MyCouponsPage() {
  const { user, isBranchMerchant } = useAuthStore();
  const merchantId = user?.merchant?.id;
  const merchantType = user?.merchant?.type;
  const isBranch = isBranchMerchant();
  const isOrganization = merchantType === 'organization' && !isBranch;
  const branchCanManageCoupons = isBranch && !!user?.merchant?.allow_branch_coupon_management;
  const canManage = !isBranch || branchCanManageCoupons;
  ...
  const { data, isLoading, refetch, isFetching } = useMyCoupons(queryParams);
  const createMutation = useCreateMyCoupon();
  const updateMutation = useUpdateMyCoupon();
  const deleteMutation = useDeleteMyCoupon();

  // Load branches for org merchants (for target branch dropdown)
  const { data: branchesData } = useMyBranches(
    isOrganization ? { per_page: 100 } : undefined
  );
  const branches = useMemo(() => {
    if (!isOrganization || !branchesData?.data) return undefined;
    return branchesData.data.map((b) => ({ id: b.id, name: b.name }));
  }, [isOrganization, branchesData]);
  ...
  const baseColCount = 6; // Code, Name, Discount, Usage, Status, Dates
  const colCount = baseColCount + (isOrganization ? 1 : 0) + (canManage ? 1 : 0);
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">
            {isBranch ? 'Coupons' : 'My Coupons'}
          </h1>
          <p className="text-muted-foreground">
            {isBranch
              ? branchCanManageCoupons
                ? 'Organization coupons and your own coupons'
                : 'Coupons managed by your organization'
        ...
        <CardHeader className="border-b">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <CardTitle>{isBranch ? (branchCanManageCoupons ? 'All Coupons' : 'Organization Coupons') : 'Your Coupons'}</CardTitle>
                <CardDescription>
                  {isBranch
                    ? branchCanManageCoupons
                      ? 'Inherited coupons from your organization and your own branch coupons.'
                      : 'These coupons are managed by your organization and apply to your branch.'
                    : 'Manage coupons for your store. Public coupons appear on your storefront.'}
                </CardDescription>
              </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                {isOrganization && <TableHead>Target</TableHead>}
                  ...
                  <TableCell colSpan={colCount} className="h-32">
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {isBranch
                          ? 'Your organization has not created any coupons yet'
                          : 'Create your first coupon to attract customers'}
                      </p>
                    </div>
                  </TableCell>
                </TableRow>
                  ...
                  <TableRow key={item.id} className="group">
                    ...
                    <TableCell>
                      ...
                      <span className="text-muted-foreground text-xs ml-1">
                        ({discountTypeLabels[item.discount_type]})
                      </span>
                    </TableCell>
                    {isOrganization && (
                      <TableCell>
                        {item.target_merchant ? (
                          <Badge variant="secondary" className="text-xs">{item.target_merchant.name}</Badge>
                        ) : (
                    ...
                    <TableCell className="text-muted-foreground text-xs">
                      <div>From: {item.starts_at ? formatDate(item.starts_at) : '-'}</div>
                      <div>To: {item.expires_at ? formatDate(item.expires_at) : 'No expiry'}</div>
                    </TableCell>
                    {canManage && (
                      <TableCell className="text-right">
                        {(!isBranch || !item.is_inherited) ? (
                          <DropdownMenu>
      ...
      {canManage && (
        <>
          <CouponFormDialog
            ...
            title="Create Coupon"
            description={isBranch ? "Create a coupon for your branch." : "Create a new coupon for your store."}
            branches={branches}
          />

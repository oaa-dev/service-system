...
import { useUpdateServiceCategory } from '@/hooks/useServiceCategories';
import { updateServiceCategorySchema, type UpdateServiceCategoryFormData } from '@/lib/validations';
import { ServiceCategory, ApiError } from '@/types/api';
...
interface Props { merchantId: number; item: ServiceCategory | null; open: boolean; onOpenChange: (open: boolean) => void; }

export function EditServiceCategoryDialog({ merchantId, item, open, onOpenChange }: Props) {
  const mutation = useUpdateServiceCategory();
  const form = useForm<UpdateServiceCategoryFormData>({
  ...
  useEffect(() => {
    if (item && open) {
      form.reset({ name: item.name, description: item.description || '', is_active: item.is_active, sort_order: item.sort_order });
    }
  }, [item, open, form]);

  const onSubmit = (data: UpdateServiceCategoryFormData) => {
    if (!item) return;
    mutation.mutate({ merchantId, id: item.id, data }, {
      onSuccess: () => onOpenChange(false),
      onError: (error) => {
        const axiosError = error as AxiosError<ApiError>;
        if (axiosError.response?.data?.errors) {
          Object.entries(axiosError.response.data.errors).forEach(([key, value]) => {
            form.setError(key as keyof UpdateServiceCategoryFormData, { message: Array.isArray(value) ? value[0] : value });
          });
        } else {
          form.setError('root', { message: axiosError.response?.data?.message || 'Failed to update service category' });
        }
      },
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) form.reset(); onOpenChange(v); }}>
      <DialogContent>
        <DialogHeader><DialogTitle>Edit Service Category</DialogTitle><DialogDescription>Update service category details.</DialogDescription></DialogHeader>
        <Form {...form}>

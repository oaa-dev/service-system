...
interface Props { merchantId: number; item: Service | null; open: boolean; onOpenChange: (open: boolean) => void; }

export function EditServiceDialog({ merchantId, item, open, onOpenChange }: Props) {
  ...
  const onSubmit = (data: UpdateServiceFormData) => {
    if (!item) return;
    mutation.mutate({ merchantId, serviceId: item.id, data }, {

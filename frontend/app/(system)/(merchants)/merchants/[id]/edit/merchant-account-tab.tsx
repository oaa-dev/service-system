...
import { z } from 'zod';
import { useUpdateMerchantAccount } from '@/hooks/useMerchants';
import { Merchant, ApiError } from '@/types/api';
...
export function MerchantAccountTab({ merchant }: Props) {
  ...
  const onSubmit = (data: AccountFormData) => {
    ...
    if (data.email !== merchant.user?.email) {
    ...
    updateMutation.mutate({ id: merchant.id, data: payload }, {
      onSuccess: () => {
  ...
  return (
    <Card className="mt-6">
      <CardHeader>
        ...
        <CardDescription>Update the merchant owner&apos;s login credentials</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="rounded-lg border p-4 space-y-1">
              ...
              <p className="text-sm text-muted-foreground">{merchant.user?.name} ({merchant.user?.email})</p>
            </div>
